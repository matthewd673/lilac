...
module Lilac
  module Analysis
    ...
    class DFA
      ...
      # represent a specific analysis. These subclasses should then expose a
      # much simpler constructor.
      #
      # @param [Direction] direction The direction of the DFA.
      # @param [T::Set[Domain]] boundary The set of domain objects assigned to
      #   either OUT[ENTRY] or IN[EXIT] (depending on direction) when a run
      #   begins.
      # @param [T::Set[Domain]] init The set of domain objects assigned to
      #   IN[block] or OUT[block] (depending on direction) for every block when
      #   a run begins.
      ...
      #
      # @return [CFGFacts[Domain]] The facts generated by the analysis.
      #   By default, DFAs output facts about each block's IN, OUT, GEN, and
      #   KILL sets which are stored in symbols +:in+, +:out+, +:gen+, and
      #   +:kill+ in this +CFGFacts+ object.
      def run
        # run the dfa
        run_dfa
      ...
      sig do
        params(hash: T::Hash[BB, T::Set[Domain]], block: BB)
